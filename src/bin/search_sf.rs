...
// Run the witness_gen binary and return the std output
...
fn main() {
  ...
  // In each iteration, we modify the config file with the new scale factor, which is
  ...
  while min_sf <= max_sf && prev_sfs.iter().find(|&&x| x == current_sf).is_none() {
    ...
    if stdout.contains("Witness generation done") {
      // If the output contains "Witness generation done", then the
      // optimal scale factor may be larger than the current scale factor
      // Set the minimum scale factor to the current scale factor
      min_sf = current_sf;
      opt_sf = current_sf;
    } else {
      // If the output does not contain "Witness generation done", then the current scale factor too high
  ...
  // Modify the config file with the optimal scale factor

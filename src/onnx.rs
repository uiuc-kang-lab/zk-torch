...
pub fn load_file(filename: &str) -> (Graph, Vec<ArrayD<Fr>>) {
  ...
  for node in onnx_graph.node.iter().filter(|node| node.op_type.as_str() != "Constant") {
    ...
    let (mut local_graph, output_shapes) = match op {
      ...
      "Ceil" => Ok(CeilLayer::graph(&input_shapes, &my_constants, &my_attributes)),
      ...
      "Range" => Ok(RangeLayer::graph(&input_shapes, &my_constants, &my_attributes)),
      "ScatterND" => Ok(ScatterNDLayer::graph(&input_shapes, &my_constants, &my_attributes)),
      "Slice" => Ok(SliceLayer::graph(&input_shapes, &my_constants, &my_attributes)),

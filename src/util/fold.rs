...
pub struct AccHolder<P: Copy, Q: Copy> {
  ...
  pub acc_fr: Vec<Fr>,
  pub mu: Fr,
  pub errs: Vec<(Vec<P>, Vec<Q>, Vec<Fr>, Vec<PairingOutput<Bls12_381>>)>, // i-th element contains err_i: [e_j]_j=1..n
  pub acc_errs: Vec<(Vec<P>, Vec<Q>, Vec<Fr>, Vec<PairingOutput<Bls12_381>>)>, // i-th element contains acc_err_i += SUM{acc_gamma^j * e_j} for j=1..n
}

pub fn acc_to_acc_proof<P: Copy, Q: Copy>(acc: AccHolder<P, Q>) -> (Vec<P>, Vec<Q>, Vec<Fr>, Vec<PairingOutput<Bls12_381>>) {
...
pub trait AccProofLayout: BasicBlock {
  ...
  fn prover_dummy_holder(&self) -> AccHolder<G1Projective, G2Projective> {
    let errs: Vec<_> = self.err_g1_nums().iter().enumerate().map(|(i, v)| (vec![G1Projective::zero(); *v], vec![G2Projective::zero(); self.err_g2_nums()[i]], vec![Fr::zero(); self.err_fr_nums()[i]], vec![PairingOutput::<Bls12_381>::zero(); self.err_gt_nums()[i]])).collect();
    AccHolder {
  ...
  fn verifier_dummy_holder(&self) -> AccHolder<G1Affine, G2Affine> {
    let errs: Vec<_> = self.err_g1_nums().iter().enumerate().map(|(i, v)| (vec![G1Affine::zero(); *v], vec![G2Affine::zero(); self.err_g2_nums()[i]], vec![Fr::zero(); self.err_fr_nums()[i]], vec![PairingOutput::<Bls12_381>::zero(); self.err_gt_nums()[i]])).collect();
    AccHolder {
...
pub fn acc_proof_to_acc<P: Copy, Q: Copy>(
  ...
  acc_proof: (&Vec<P>, &Vec<Q>, &Vec<Fr>, &Vec<PairingOutput<Bls12_381>>),
  is_prover: bool,
) -> AccHolder<P, Q> {
  ...
  for i in 0..bb.err_g1_nums().len() {
    ...
    let err: (Vec<P>, Vec<Q>, Vec<Fr>, Vec<PairingOutput<Bls12_381>>) = (

...
impl BasicBlock for MaxProofBasicBlock {
  ...
  fn prove(
    ...
    #[cfg(feature = "gpu")]
    let s_x = util::gpu_msm_for_x1a(&cache, &srs.IX1A as &Vec<IG1A>, 0, s_poly.coeffs.len(), &srs.X1A, &s_poly.coeffs);
    ...
    #[cfg(feature = "gpu")]
    let W_x = util::gpu_msm_for_x1a(&cache, &srs.IX1A as &Vec<IG1A>, 0, W_Q.coeffs.len(), &srs.X1A, &W_Q.coeffs);
    ...
    #[cfg(feature = "gpu")]
    let W_gx = util::gpu_msm_for_x1a(&cache, &srs.IX1A as &Vec<IG1A>, 0, W_gQ.coeffs.len(), &srs.X1A, &W_gQ.coeffs);

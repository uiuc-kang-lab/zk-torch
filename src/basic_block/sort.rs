...
impl BasicBlock for SortBasicBlock {
  fn run(&self, _model: &ArrayD<Fr>, inputs: &Vec<&ArrayD<Fr>>) -> Vec<ArrayD<Fr>> {
    ...
    let mut sorted_data_shift_1 = util::vec_iter(&sorted_data).skip(1).cloned().collect::<Vec<_>>();
    ...
    let d = (sorted_data.clone() - sorted_data_shift_1).into_dyn();
  ...
  fn prove(
    ...
    let r: Vec<_> = (0..21).map(|_| Fr::rand(&mut rng2)).collect();
    ...
    let sorted_data = outputs[0].first().unwrap().raw.clone();
    let sorted_indices = outputs[1].first().unwrap().raw.clone();
    ...
    let g_s_poly = outputs[0].first().unwrap().poly.clone().add(r_f_s_poly.mul_by_vanishing_poly(domain));
    ...
    let g_idx_s_poly = outputs[1].first().unwrap().poly.clone().add(r_f_idx_s_poly.mul_by_vanishing_poly(domain));
    ...
    let r_diff_poly = DensePolynomial {
      coeffs: vec![r[12], r[13], r[14]],
    };
    let g_diff_poly = outputs[2].first().unwrap().poly.clone().add(r_diff_poly.mul_by_vanishing_poly(domain));
    let g_diff_x = util::msm::<G1Projective>(&srs.X1A, &g_diff_poly.coeffs);

    // compute q(x) = [f(x) - g(x)]/ (x^N - 1) for proving f(x) = g(x)
    let q_poly = inputs[0].first().unwrap().poly.clone().sub(&g_poly.clone()).divide_by_vanishing_poly(domain).unwrap().0;
    let r_Q = r[15];
    ...
    let q_idx_poly = inputs[1].first().unwrap().poly.clone().sub(&g_idx_poly.clone()).divide_by_vanishing_poly(domain).unwrap().0;
    let r_Q_idx = r[16];
    ...
    let q_s_poly = outputs[0].first().unwrap().poly.clone().sub(&g_s_poly.clone()).divide_by_vanishing_poly(domain).unwrap().0;
    let r_Q_s = r[17];
    ...
    let q_idx_s_poly = outputs[1].first().unwrap().poly.clone().sub(&g_idx_s_poly.clone()).divide_by_vanishing_poly(domain).unwrap().0;
    let r_Q_idx_s = r[18];
    ...
    let r_idx_s_plus_r_Q_idx_s = DensePolynomial { coeffs: vec![r_Q_idx_s] }.mul_by_vanishing_poly(domain).sub(&DensePolynomial {
      coeffs: vec![outputs[1].first().unwrap().r],
    });
    ...
    let q_diff_poly = &outputs[2].first().unwrap().poly.clone().sub(&g_diff_poly.clone()).divide_by_vanishing_poly(domain).unwrap().0;
    let r_Q_diff = r[19];
    ...
    let r_diff_plus_r_Q_diff = DensePolynomial { coeffs: vec![r_Q_diff] }.mul_by_vanishing_poly(domain).sub(&DensePolynomial {
      coeffs: vec![outputs[2].first().unwrap().r],
    });
    ...
    let mut rng2 = StdRng::from_entropy();
    let mut proof = vec![g_x, g_idx_x, g_s_x, g_idx_s_x, g_diff_x, q_x, q_idx_x, q_s_x, q_idx_s_x, q_diff_x];
    ...
    let t_N_poly = &Z_omega_poly.mul(&(&g_s_poly + &beta_poly.mul(&g_idx_s_poly).add(gamma_poly.clone())))
      ...
      + LN_minus_one_poly.mul(&g_diff_poly.clone().add(g_s_omega_poly.clone()).sub(&g_s_poly)).mul(&alpha_poly).mul(&alpha_poly);
    ...
    // Fiat-Shamir
    let mut bytes = Vec::new();
    let mut proof_1 = vec![t_x + srs.Y1P * r[20]];
    ...
    let h_numerator_terms = vec![
      ...
      g_diff_poly.clone().sub(&evals_polys[8].clone()),
    ];
    ...
    let h_prime_numerator_terms = vec![Z_poly.clone().sub(&evals_polys[2].clone()), g_s_poly.clone().sub(&evals_polys[7].clone())];
    ...
    let _u = Fr::rand(rng);
    let w = Fr::rand(rng);
    let w_pows = calc_pow(w, 4);
    proof.append(&mut proof1);

    // blinding
    let mut C = r_plus_r_Q_x
      + r_idx_plus_r_Q_idx_x * w_pows[0]
      + r_s_plus_r_Q_s_x * w_pows[1]
      + r_idx_s_plus_r_Q_idx_s_x * w_pows[2]
      + r_diff_plus_r_Q_diff_x * w_pows[3];
    C += srs.X1P[0] * r[20] * v_pows[0];
    proof.push(C);
  ...
  fn verify(
    ...
    let sorted_data = outputs[0].first().unwrap().g1;
    let sorted_indices = outputs[1].first().unwrap().g1;
    let d = outputs[2].first().unwrap().g1;

    let [g_x, g_idx_x, g_s_x, g_idx_s_x, g_diff_x, q_x, q_idx_x, q_s_x, q_idx_s_x, q_diff_x, Z_x, t_x, h_x, h_prime_x, C] = proof.0[..] else {
    ...
    // Round 2 randomness
    let mut bytes = Vec::new();
    vec![g_x, g_idx_x, g_s_x, g_idx_s_x, g_diff_x, q_x, q_idx_x, q_s_x, q_idx_s_x, q_diff_x]
    ...
    //   + [L_{N-1}(zeta)-1][d(zeta) + g_s(omega * zeta) - g_s(zeta)] * alpha^2
    ...
    let A_zeta = Z_omega_zeta * (g_s_zeta + beta * g_idx_s_zeta + gamma) - Z_zeta * (g_zeta + beta * g_idx_zeta + gamma)
      ...
      + (LN_zeta - Fr::one()) * (g_diff_zeta + g_s_omega_zeta - g_s_zeta) * alpha * alpha;
    assert!(t_zeta * vanishing_poly_zeta == A_zeta);

    // Verify openings are correct
    let v_pows = calc_pow(v, 9);
    let mut check_for_opening_at_zeta =
      t_x * v_pows[0] + Z_x * v_pows[1] + g_x * v_pows[2] + g_idx_x * v_pows[3] + g_s_x * v_pows[4] + g_idx_s_x * v_pows[5] + g_diff_x * v_pows[6];
    check_for_opening_at_zeta -= srs.X1A[0]
      * (t_zeta * v_pows[0]
        ...
        + g_idx_zeta * v_pows[3]
        + g_s_zeta * v_pows[4]
        + g_idx_s_zeta * v_pows[5]
        + g_diff_zeta * v_pows[6]);
    let mut check_for_opening_at_omega_zeta = Z_x * v_pows[0] + g_s_x * v_pows[1];
    check_for_opening_at_omega_zeta -= srs.X1A[0] * (Z_omega_zeta * v_pows[0] + g_s_omega_zeta * v_pows[1]);
    let check_for_g_eq_f = (data - g_x)
      + (indices - g_idx_x) * w_pows[0]
      + (sorted_data - g_s_x) * w_pows[1]
      + (sorted_indices - g_idx_s_x) * w_pows[2]
      + (d - g_diff_x) * w_pows[3];
    let check_for_g_eq_f_q_terms = q_x + q_idx_x * w_pows[0] + q_s_x * w_pows[1] + q_idx_s_x * w_pows[2] + q_diff_x * w_pows[3];

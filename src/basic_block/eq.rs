...
impl BasicBlock for ElementwiseEqBasicBlock {
  fn run(&self, _model: &ArrayD<Fr>, inputs: &Vec<&ArrayD<Fr>>) -> Vec<ArrayD<Fr>> {
    ...
    if inputs[0].len() == 1 && inputs[1].ndim() > 0 { // broadcast inputs[0] to compare with each element in inputs[1]
      Zip::from(r.view_mut())
        .and(inputs[1].view())
        .for_each(|r, &x| *r = (util::fr_to_int(x) == util::fr_to_int(*inputs[0].first().unwrap())) as u8);
    } else if inputs[1].len() == 1 && inputs[0].ndim() > 0 { // broadcast inputs[1] to compare with each element in inputs[0]
      Zip::from(r.view_mut())
        .and(inputs[0].view())

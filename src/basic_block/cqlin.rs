...
use ark_std::{iterable::Iterable, UniformRand, Zero};
use rand::Rng;
use rayon::prelude::*;
...
impl BasicBlock for CQLinBasicBlock {
  fn run(model: &Data, inputs: &Vec<Vec<Fr>>) -> Vec<Fr> {
    ...
    for i in 0..n {
      for j in 0..m {
        r[i] += model.raw[j * n + i] * inputs[0][j];
      }
    }
    r
  }
  fn setup(srs: (&Vec<G1Affine>, &Vec<G2Affine>), model: &Data) -> (Vec<G1Affine>, Vec<G2Affine>) {
    ...
    let mut V = srs_p[n * m - n..n * m].to_vec();
    ...
    let S: Vec<Vec<_>> = (0..m)
      .into_par_iter()
      .map(|i| (0..n).map(|j| (U[i][j] * domain_m.element(i).inverse().unwrap() - V[j]) * model.raw[i * n + j]).collect())
      .collect();
    let S: Vec<_> = S.par_iter().map(|x| x.iter().sum::<G1Projective>()).collect();

    let R: Vec<Vec<_>> = (0..m).into_par_iter().map(|i| (0..n).map(|j| U[i][j] * model.raw[i * n + j]).collect()).collect();
    let R: Vec<_> = R.par_iter().map(|x| x.iter().sum::<G1Projective>()).collect();

    let p_R: Vec<Vec<_>> = (0..m).into_par_iter().map(|i| (0..n).map(|j| U_p_R[i][j] * model.raw[i * n + j]).collect()).collect();
    let p_R: Vec<_> = p_R.par_iter().map(|x| x.iter().sum::<G1Projective>()).collect();

    let mut C: Vec<Vec<_>> = (0..n).into_par_iter().map(|i| (0..m).map(|j| model.raw[j * n + i]).collect()).collect();
    ...
    let M_x = (0..m).into_par_iter().map(|i| (0..n).map(|j| U2[i][j] * model.raw[i * n + j]).sum::<G2Projective>()).sum::<G2Projective>(); //TODO: Change to msm
  ...
  fn prove<R: Rng>(
    ...
    (vec![R_x, Q_x, A_x, S_x, P_x, z, p_R_x, pi, pi_1], vec![setup.1[0]])
  }
  fn verify<R: Rng>(
    srs: (&Vec<G1Affine>, &Vec<G2Affine>),
    model: &DataEnc,
    ...
    let [R_x, Q_x, A_x, S_x, P_x, z, p_R_x, pi, pi_1] = proof.0[..] else {

...
impl BasicBlock for CQLinBasicBlock {
  ...
  fn setup(&self, srs: &SRS, model: &ArrayD<Fr>) -> SetupType {
    let weights = convert_to_data(srs, model);
    let m = weights.len();
  ...
  fn prove(
    ...
    let setup = if let SetupType::CQLin(cqlin_setup) = setup {
      cqlin_setup
    } else {
      panic!("CQLin has an incorrect SetupType");
    };

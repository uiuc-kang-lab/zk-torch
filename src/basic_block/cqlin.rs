...
impl BasicBlock for SparseCQLinBasicBlock {
  ...
  fn setup(&self, srs: &SRS, _model: &ArrayD<Data>) -> (Vec<G1Projective>, Vec<G2Projective>, Vec<DensePolynomial<Fr>>) {
    ...
    let S: Vec<Vec<_>> = (0..m)
      ...
      .map(|i| {
        (0..n)
          .map(|j| (U[i][j] * domain_m.element(i).inverse().unwrap() - V[j]) * get_model_element(&self.kernel, &self.indices, i, j))
          .collect()
      })
      .collect();
    ...
    let R: Vec<Vec<_>> =
      (0..m).into_par_iter().map(|i| (0..n).map(|j| U[i][j] * get_model_element(&self.kernel, &self.indices, i, j)).collect()).collect();
    let R: Vec<_> = R.par_iter().map(|x| x.iter().sum::<G1Projective>()).collect();

    let P_R: Vec<Vec<_>> = (0..m)
      .into_par_iter()
      .map(|i| (0..n).map(|j| U_P_R[i][j] * get_model_element(&self.kernel, &self.indices, i, j)).collect())
      .collect();
    let mut P_R: Vec<_> = P_R.par_iter().map(|x| x.iter().sum::<G1Projective>()).collect();

    // Calculate C for Q
    let mut C: Vec<Vec<_>> = (0..n).into_par_iter().map(|i| (0..m).map(|j| get_model_element(&self.kernel, &self.indices, j, i)).collect()).collect();
    ...
    let M_x = (0..m)
      .into_par_iter()
      .map(|i| (0..n).map(|j| U2[i][j] * get_model_element(&self.kernel, &self.indices, i, j)).sum::<G2Projective>())
      .sum::<G2Projective>(); // TODO: Change to msm

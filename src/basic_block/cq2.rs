...
impl BasicBlock for CQ2BasicBlock {
  ...
  fn setup(&self, srs: &SRS, model: &ArrayD<Data>) -> (Vec<G1Projective>, Vec<G2Projective>, Vec<DensePolynomial<Fr>>) {
    ...
    for i in 0..2 {
      ...
      #[cfg(feature = "gpu")]
      setup2.push(util::gpu_msm_g2(&srs.IX2A as &Vec<IG2A>, &model[i].poly.coeffs as &Vec<Fr>) + srs.Y2P * model[i].r);
  ...
  fn prove(
    ...
    #[cfg(feature = "gpu")]
    let f_x_2 = util::gpu_msm_g2(&srs.IX2A as &Vec<IG2A>, &agg_input.poly.coeffs as &Vec<Fr>) + srs.Y2P * agg_input.r;

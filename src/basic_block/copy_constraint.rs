...
fn construct_ssig_coeffs(idxs: &[(usize, usize)], partitions: &HashMap<usize, Vec<usize>>, is_input: bool) -> Vec<Fr> {
  idxs
    ...
    .flat_map(|(idx, perm_idx)| {
      ...
      let sigma = {
        let partition = partitions.get(&inp_idx).ok_or_else(|| format!("Key {:?} not found in the map", inp_idx)).unwrap();
...
impl BasicBlock for CopyConstraintBasicBlock {
  ...
  fn prove(
    ...
    proof.append(&mut vec![q1_Q_x, Z_Q_x, r_Q_x]);
    let mut proof: Vec<_> = proof.iter().enumerate().map(|(i, x)| (*x) + srs.Y1P * r1[i]).collect();
    proof.append(&mut C);

...
impl BasicBlock for AddBasicBlock {
  fn run(&self, _model: &ArrayD<Fr>, inputs: &Vec<&ArrayD<Fr>>) -> Result<Vec<ArrayD<Fr>>, util::CQOutOfRangeError> {
    ...
    if inputs[1].ndim() == inputs[0].ndim() && inputs[0].ndim() == 0 && inputs[1].len() == 1 && inputs[0].len() == 1 {
      return Ok(vec![(inputs[0] + inputs[1]).to_owned()]);
    }
    let mut r = ArrayD::zeros(IxDyn(&[std::cmp::max(inputs[0].len(), inputs[1].len())]));
    if inputs[1].len() == 1 && inputs[0].len() == 1 {
      r[0] = inputs[0].first().unwrap() + inputs[1].first().unwrap();
    } else if inputs[0].len() == 1 && inputs[1].ndim() > 0 {

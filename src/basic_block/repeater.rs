...
fn repeater_prove_wrapper(
  ...
  ndarr_azip!(((localInputs, localOutputs) in &mut temp, x in &mut empty) {
    ...
    let tmp = basic_block.prove(srs, setup, model, &localInputs, &localOutputs, &mut rng, cache.clone());
    *x = tmp;
  });
...
fn repeater_verify_wrapper(
  ...
  ndarr_azip!(((localInputs, localOutputs) in &mut temp, localProof in &mut proofArr, x in &mut empty){
    ...
    let mut rng = rng.clone();
    let temp = basic_block.verify(srs, model, &localInputs, &localOutputs, localProof, &mut rng, cache.clone());
    *x = temp;
  });

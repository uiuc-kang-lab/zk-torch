...
impl MatMultBasicBlock {
  pub fn run(&self, inputs: &Vec<&ArrayD<Fr>>) -> ArrayD<Fr> {
    ...
    ArrayD::from_shape_vec(vec![M, K], r).unwrap()
  }
  pub fn prove(
    &self,
    ...
    let time_matrix_padding = start.elapsed();
    println!("Matrix padding time: {:?}", time_matrix_padding);
    ...
    let tho = Fr::rand(rng);
    let time_random_point = start.elapsed();
    println!("Random point generation time: {:?}", time_random_point - time_matrix_padding);
    ...
    let time_eval_to_poly = start.elapsed();
    println!("Evaluation to polynomial time: {:?}", time_eval_to_poly - time_random_point);
    ...
    let time_commitment = start.elapsed();
    println!("Commitment time: {:?}", time_commitment - time_eval_to_poly);
    ...
    let time_misc = start.elapsed();
    println!("Misc time: {:?}", time_misc - time_commitment);

    let (sumcheck_proof, _prover_state) = MLSumcheck::prove_as_subprotocol(&mut fs_rng, &poly).expect("fail to prove");
    let time_proof = start.elapsed();
    println!("Sumcheck time: {:?}", time_proof - time_misc);
    ...
    println!("Verify time: {:?}", time_verify - time_proof);
    ...
    let time_zm_proof = start.elapsed();
    println!("ZM proof time: {:?}", time_zm_proof - time_verify);
  ...
  pub fn verify(
    ...
    let M = inputs[0].raw.shape()[0];
    let K = inputs[1].raw.shape()[1];

    let m = (M as f64).log2() as usize + 1;
    let k = (K as f64).log2() as usize + 1;
    ...
    assert!(
      f_a_zm_proof.value * f_b_zm_proof.value + tho * random_poly_zm_proof.value == expected_evaluation,
      "wrong subclaim"
    );
    ...
    util::zm_verify(
      srs,
      util::NV_MAX,
      random_poly_zm_proof.point.len(),
      com_random_poly,
      random_poly_zm_proof,
      rng,
    );

...
// Return the "indices" field in SparseCQLinBasicBlock for a given input and kernel shape
...
// Slice a kernel map to a smaller kernel map
...
// Return (m, n), where the optimal divided submatrices for CQLin are of size 2^m x 2^n
...
// 2D input: [B, C_in * H_in * W_in]
// 2D output: [B, C_out * H_out * W_out]

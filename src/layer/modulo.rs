...
impl Layer for ModLayer {
  fn graph(input_shapes: &Vec<&Vec<usize>>, constants: &Vec<Option<&ArrayD<Fr>>>, _attributes: &Vec<&AttributeProto>) -> (Graph, Vec<Vec<usize>>) {
    ...
    // currently the length of the second input can only be 1
    if let Some(c) = constants[1] {
      assert!(c.len() == 1);
      let c = util::fr_to_int(*c.first().unwrap()) as u32;
      let modc = graph.addBB(Box::new(ModConstBasicBlock { c: c }));
      let mod_check = graph.addBB(Box::new(RepeaterBasicBlock {
        basic_block: Box::new(CQ2BasicBlock {
          setup: Some((Box::new(ModConstBasicBlock { c: c }), onnx::CQ_RANGE_LOWER, onnx::CQ_RANGE)),
        }),
        N: 1,
      }));
      let mod_output = graph.addNode(modc, vec![(-1, 0)]);
      let _ = graph.addNode(mod_check, vec![(-1, 0), (mod_output, 0)]);
      graph.outputs.push((mod_output, 0));
    ...
    // Finally, push the remainder to the output
    graph.outputs.push((div_output, 1));

...
impl Layer for SoftmaxLayer {
  fn graph(input_shapes: &Vec<&Vec<usize>>, _constants: &Vec<Option<&ArrayD<Fr>>>, _attributes: &Vec<&AttributeProto>) -> (Graph, Vec<Vec<usize>>) {
    ...
    let exp = graph.addBB(Box::new(ExpBasicBlock { input_SF: onnx::SF_LOG, output_SF: onnx::SF_LOG }));
    ...
    let log = graph.addBB(Box::new(LogBasicBlock { input_SF: onnx::SF_LOG, output_SF: onnx::SF_LOG }));
